...
2. The server sends ``mode <mode>`` where ``<mode>`` is either ``local`` if the server runs a local game or ``normal`` if the server runs a regular server.
    ...
    b. If ``<mode>`` is ``normal``, then the server sends ``color <color>``, ``<color>`` is whatever color the client is assigned.
...
4. Finally, the server sends ``ready`` to indicate it is ready for subsequent communication.
